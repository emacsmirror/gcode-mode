...
(defun gcode-mode--doc-entries () '(
  ...
  ("G0" "Linear Move" ?E "The length of filament to feed into the extruder between the start and end point" ?F "The maximum movement rate of the move between the start and end point. The feedrate set here applies to subsequent moves that omit this parameter" ?X "A coordinate on the X axis" ?Y "A coordinate on the Y axis" ?Z "A coordinate on the Z axis")
  ("G1" "Linear Move" ?E "The length of filament to feed into the extruder between the start and end point" ?F "The maximum movement rate of the move between the start and end point. The feedrate set here applies to subsequent moves that omit this parameter" ?X "A coordinate on the X axis" ?Y "A coordinate on the Y axis" ?Z "A coordinate on the Z axis")
  ...
  ("G5" "Bézier cubic spline" ?E "The length of filament to feed into the extruder between the start and end point" ?F "The maximum feedrate of the move between the start and end point (in current units per second). This value applies to all subsequent moves" ?I "X incremental offset from start point to first control point" ?J "Y incremental offset from start point to first control point" ?P "X incremental offset from end point to second control point" ?Q "Y incremental offset from end point to second control point" ?X "A coordinate on the X axis" ?Y "A coordinate on the Y axis")
  ("G6" "Direct Stepper Move")
  ("G10" "Retract" ?S "Use G10 S1 to do a swap retraction, before changing extruders. The subsequent G011 (after tool change) will do a swap recover. (Requires EXTRUDERS > 1)")
  ("G11" "Recover")
  ...
  ("G26" "Mesh Validation Pattern" ?B "Bed temperature (otherwise 60°C) to use for the test print" ?C "Continue with the closest point (otherwise, don't)" ?D "Disable leveling compensation (otherwise, enable)" ?F "Filament diameter (otherwise 1.75mm)" ?H "Hot end temperature (otherwise 205°C) to use for the test print" ?I "Material preset to use for the test print. Overrides S" ?K "Keep heaters on when done" ?L "Layer height to use for the test" ?O "Ooze amount (otherwise 0.3mm). Emitted at the start of the test" ?P "Prime Length" ?Q "Retraction multiplier (otherwise 1.0)" ?R "Number of G026 Repetitions (otherwise 999)" ?S "Nozzle size (otherwise 0.4mm)" ?U "Random deviation. (U with no value, 50)" ?X "X position (otherwise, current X position)" ?Y "Y position (otherwise, current Y position)")
  ("G27" "Park toolhead" ?P "Z axis action")
  ("G28" "Auto Home" ?O "Optional. If the position is known then exit without homing. (1.1.9)" ?R "Raise before homing distance (1.1.9)" ?X "Flag to go back to the X axis origin" ?Y "Flag to go back to the Y axis origin" ?Z "Flag to go back to the Z axis origin")
  ("G29" "Bed Leveling")
  ("G29" "Bed Leveling (3-Point)" ?A "Abort leveling procedure in-progress (PROBE_MANUALLY)" ?C "Create fake points for testing. (DEBUG_LEVELING_FEATURE)" ?D "Dry-Run mode. Just probe the grid but don't update the bed leveling data" ?E "- By default G29 will engage the Z probe, test the bed, then disengage" ?J "Jettison the leveling data stored in SRAM and turn off leveling compensation. Data in EEPROM is not affected" ?O "Optional. If leveling is already enabled then exit without leveling. (1.1.9)" ?Q "Query the current leveling state (PROBE_MANUALLY, DEBUG_LEVELING_FEATURE)" ?V "Set the verbose level")
  ("G29" "Bed Leveling (Bilinear)" ?A "Abort leveling procedure in-progress (PROBE_MANUALLY)" ?B "Set the back limit of the probing grid" ?C "Create a fake grid for testing. (DEBUG_LEVELING_FEATURE)" ?D "Dry-Run mode. Just probe the grid but don't update the bed leveling data" ?E "- By default G29 will engage the Z probe, test the bed, then disengage" ?F "Set the front limit of the probing grid" ?H "Set the square width and height of the area to probe" ?J "(Without W) Jettison the leveling data stored in SRAM and turn off leveling compensation. Data in EEPROM is not affected" ?L "Set the left limit of the probing grid" ?O "Optional. If leveling is already enabled then exit without leveling. (1.1.9)" ?Q "Query the current leveling state (PROBE_MANUALLY, DEBUG_LEVELING_FEATURE)" ?R "Set the right limit of the probing grid" ?S "Set the XY travel speed between probe points" ?V "Set the verbose level" ?W "Write a mesh Z offset" ?X "Specify X when setting a mesh value (PROBE_MANUALLY)" ?Y "Specify Y when setting a mesh value" ?Z "Specify the Z offset when setting a mesh value")
  ("G29" "Bed Leveling (Linear)" ?A "Abort leveling procedure in-progress (PROBE_MANUALLY)" ?B "Set the back limit of the probing grid" ?C "Create a fake grid for testing. (DEBUG_LEVELING_FEATURE)" ?D "Dry-Run mode. Just probe the grid but don't update the bed leveling data" ?E "- By default G29 will engage the Z probe, test the bed, then disengage" ?F "Set the front limit of the probing grid" ?H "Set the square width and height of the area to probe" ?J "Jettison the leveling data stored in SRAM and turn off leveling compensation. Data in EEPROM is not affected" ?L "Set the left limit of the probing grid" ?O "Optional. If leveling is already enabled then exit without leveling. (1.1.9)" ?P "Set the size of the square grid that will be probed - P x P points" ?Q "Query the current leveling state (PROBE_MANUALLY, DEBUG_LEVELING_FEATURE)" ?R "Set the right limit of the probing grid" ?S "Set the XY travel speed between probe points" ?T "Generate a Bed Topology Report" ?V "Set the verbose level. Level 2 and up act like 'T'" ?X "Set the number of columns (in X) that will be probed" ?Y "Set the number of rows (in Y) that will be probed")
  ...
  ("G29" "Bed Leveling (Unified)" ?A "*Activate*" ?B "*Business Card* mode (P2 only)" ?C "- G29 P1 C Continue:" ?D "Disable Unified Bed Leveling (i.e., M420 S0)" ?E "Stow probe after probing Each point (P1 only)" ?F "*Fade* height. (UBL only! For others use M420 Z)" ?H "*Height*:" ?I "*Invalidate* a number of mesh points (default 1)" ?J "*Grid* (or 3-Point) leveling:" ?K "*Kompare*: Subtract (diff) the stored mesh with the given index from the current mesh. This operates on the mesh in-memory, so it will probably make the active mesh unsuitable for printing" ?L "*Load* a mesh. If no index is given, load the previously-activated mesh" ?P "*Phase*: The Phase commands are used for the bulk of the work to setup a Mesh. In general, you'll start by initializing with a G29 P0 or a G29 P1 then do further refinement with additional Phases" ?Q "Test Pattern. Load the specified Test Pattern to check for correct operation. This command is intended for developers and is not required for everyday bed leveling" ?R "Repeat count. (Default GRID_MAX_POINTS_X * GRID_MAX_POINTS_Y)" ?S "*Save* the mesh to EEPROM in the given slot" ?T "*Topology*: Include a Topology Map in the output" ?U "*Unlevel*: Probe the outer perimeter to assist bed tramming. (Use with G29 P1 O)" ?V "Verbosity Level (0-4)" ?W "*_What?_*: Display valuable UBL data" ?X "*X position* for all phases and commands (Default: current X)" ?Y "*Y position* for all phases and commands (Default: current Y)")
  ...
  ("G31" "Dock Sled")
  ("G32" "Undock Sled")
  ("G33" "Delta Auto Calibration" ?C "If omitted iterations stop at best achievable precision. If set iterations will stop at the set precision" ?E "Engage the probe for each point" ?F "Run (\"force\") this number of iterations and take the best result" ?P "Number of probe points. If not specified, uses DELTA_CALIBRATION_DEFAULT_POINTS" ?T "Disable tower angle corrections calibration (P3-P7)" ?V "Set the verbose level")
  ...
  ("G42" "Move to mesh coordinate" ?F "The maximum movement rate of the move between the start and end point. The feedrate set here applies to subsequent moves that omit this parameter" ?I "The column of the mesh coordinate" ?J "The row of the mesh coordinate")
  ...
  ("G61" "Return to Saved Position" ?F "Move feedrate" ?S "Use saved position of the specified memory slot (defaults to 0 if not spificied)" ?X "Axes to restore. At least one is required")
  ...
  ("G425" "Backlash Calibration" ?B "Perform calibration of backlash only" ?T "Perform calibration of one toolhead only" ?U "Uncertainty: how far to start probe away from the cube (mm)" ?V "Probe cube and print position, error, backlash and hotend offset. (Requires CALIBRATION_REPORTING)")
  ("M0" "Unconditional stop" ?P "Expire time, in milliseconds" ?S "Expire time, in seconds" ?s "An optional message to display on the LCD")
  ("M1" "Unconditional stop" ?P "Expire time, in milliseconds" ?S "Expire time, in seconds" ?s "An optional message to display on the LCD")
  ("M2" "Program End")
  ...
  ("M16" "Expected Printer Check" ?s "The string to compare to MACHINE_NAME")
  ("M17" "Enable Steppers" ?E "E Enable" ?X "X Enable" ?Y "Y Enable" ?Z "Z Enable")
  ("M18" "Disable steppers" ?E "E Disable" ?S "Inactivity Timeout. If none specified, disable now" ?X "X Disable" ?Y "Y Disable" ?Z "Z Disable")
  ("M20" "List SD Card")
  ("M21" "Init SD card")
  ("M22" "Release SD card")
  ("M23" "Select SD file" ?f "The filename of the file to open")
  ...
  ("M27" "Report SD print status" ?C "Report the filename and long filename of the current file" ?S "Interval between auto-reports. S0 to disable  (requires AUTO_REPORT_SD_STATUS)")
  ("M28" "Start SD write" ?f "File name to write")
  ("M29" "Stop SD write")
  ("M30" "Delete SD file" ?f "The filename of the file to delete")
  ("M31" "Print time")
  ("M32" "Select and Start" ?P "Sub-Program flag" ?S "Starting file offset")
  ("M33" "Get Long Path" ?p "DOS 8.3 path to a file or folder")
  ...
  ("M40" "Eject")
  ("M41" "Loop")
  ("M42" "Set Pin State" ?I "Ignore protection on pins that Marlin is using" ?M "Set the pin mode" ?P "A digital pin number (even for analog pins) to write to. (LED_PIN if omitted)" ?S "The state to set. PWM pins may be set from 0-255")
  ("M43" "Debug Pins" ?E "Watch endstops" ?I "Ignore protection when reporting values" ?P "Digital Pin Number" ?S "Test BLTouch type servo probes. Use P to specify servo index (0-3). Defaults to 0 if P omitted" ?T "Toggle pins - see M043-T for options" ?W "Watch pins")
  ...
  ("M48" "Probe Accuracy Test" ?E "Engage for each probe" ?L "Number of legs to probe" ?P "Number of probes to do" ?S "Star/Schizoid probe. By default this will do 7 points. Override with L" ?V "Verbose Level" ?X "X Position" ?Y "Y Position")
  ...
  ("M82" "E Absolute")
  ("M83" "E Relative")
  ("M84" "Disable steppers" ?E "E Disable" ?S "Inactivity Timeout. If none specified, disable now" ?X "X Disable" ?Y "Y Disable" ?Z "Z Disable")
  ...
  ("M100" "Free Memory" ?C "Corrupt 'n' locations in the free memory pool and report the locations of the corruption. This is useful to check the correctness of the M100 D and M100 F commands" ?D "Dump the free memory block from __brkval to the stack pointer" ?F "Return the number of free bytes in the memory pool along with other vital statistics that define the memory pool" ?I "Initialize the free memory pool so it can be watched and print vital statistics that define the free memory pool")
  ...
  ("M104" "Set Hotend Temperature" ?B "AUTOTEMP: The max auto-temperature" ?F "AUTOTEMP: Autotemp flag. Omit to disable autotemp" ?I "Material preset index. Overrides S" ?S "Target temperature.AUTOTEMP: the min auto-temperature" ?T "Hotend index. If omitted, the currently active hotend will be used")
  ("M105" "Report Temperatures" ?T "Hotend index")
  ("M106" "Set Fan Speed" ?I "Material preset index. Overrides S" ?P "Fan index" ?S "Speed, from 0 to 255. S255 provides 100% duty cycle; S128 produces 50%" ?T "Secondary speed. Added in Marlin 1.1.7. (Requires EXTRA_FAN_SPEED)")
  ("M107" "Fan Off" ?P "Fan index")
  ("M108" "Break and Continue")
  ("M109" "Wait for Hotend Temperature" ?B "With AUTOTEMP, the max auto-temperature" ?F "Autotemp flag. Omit to disable autotemp" ?I "Material preset index. Overrides S" ?R "Target temperature (wait for cooling or heating)" ?S "Target temperature (wait only when heating). Also AUTOTEMP: The min auto-temperature" ?T "Hotend index. If omitted, the currently active hotend will be used")
  ...
  ("M115" "Firmware Info")
  ("M116" "Wait")
  ("M117" "Set LCD Message" ?s "LCD status message")
  ("M118" "Serial print" ?A "Prepend // to denote a comment or action command. Hosts like OctoPrint can interpret such commands to perform special actions. See your host's documentation" ?E "Prepend echo: to the message. Some hosts will display echo messages differently when preceded by echo:" ?s "Message string. If omitted, a blank line will be sent")
  ("M119" "Endstop States")
  ("M120" "Enable Endstops")
  ("M121" "Disable Endstops")
  ("M122" "TMC Debugging" ?E "Target E driver(s) only" ?I "Flag to re-initialize stepper drivers with current settings" ?S "Flag to enable/disable continuous debug reporting" ?V "Report raw register data. Refer to the datasheet to decypher" ?X "Target X driver(s) only" ?Y "Target Y driver(s) only" ?Z "Target Z driver(s) only")
  ...
  ("M140" "Set Bed Temperature" ?I "Material preset index. Overrides S" ?S "Target temperature.AUTOTEMP: the min auto-temperature")
  ("M141" "Set Chamber Temperature" ?S "Target temperature.AUTOTEMP: the min auto-temperature")
  ...
  ("M155" "Temperature Auto-Report" ?S "Interval in seconds between auto-reports. S0 to disable")
  ...
  ("M166" "Gradient Mix" ?A "Starting Z Height. (Use Z to set the Ending Z Height.)" ?I "Starting Virtual Tool. The Gradient begins with this tool-mix. Below the Starting Z Height the Starting Virtual Tool fully applies" ?J "Ending Virtual Tool. The Gradient transitions to this tool-mix as Z approaches the Ending Z Height. Above the Ending Z Height the Ending Virtual Tool fully applies" ?S "Enable / disable the gradient in manual mode. When using the tool index alias, tool-change commands determine whether or not the gradient is enabled" ?T "A tool index to reassign to the gradient. If no index is given, cancel the tool assignment" ?Z "Ending Z Height. (Use A to set the Starting Z Height.)")
  ("M190" "Wait for Bed Temperature" ?I "Material preset index. Overrides S" ?R "Target temperature (wait for cooling or heating)" ?S "Target temperature (wait only when heating). Also AUTOTEMP: The min auto-temperature")
  ("M191" "Wait for Chamber Temperature" ?R "Target temperature (wait for cooling or heating)" ?S "Target temperature (wait only when heating). Also AUTOTEMP: The min auto-temperature")
  ("M192" "Wait for Probe temperature" ?R "Temperature to wait for, whether heating or cooling" ?S "A minimum temperature to wait for. No wait if already higher")
  ("M200" "Set Filament Diameter" ?D "Filament diameter" ?T "Extruder index. If omitted, the currently active extruder will be used")
  ...
  ("M204" "Set Starting Acceleration" ?P "Printing acceleration. Used for moves that include extrusion (i.e., which employ the current tool)" ?R "Retract acceleration. Used for extruder retraction moves" ?S "Legacy parameter for move acceleration. Set both printing and travel acceleration" ?T "Travel acceleration. Used for moves that include no extrusion")
  ...
  ("M208" "Firmware Recover" ?F "Recover feedrate (units/min)" ?R "Swap recover feedrate (units/min)" ?S "Additional recover length. Can be negative to reduce recover length" ?W "Additional recover swap length. Can be negative to reduce the length")
  ...
  ("M217" "Filament swap parameters" ?P "Prime feedrate" ?R "Retract feedrate" ?S "Swap length" ?X "Park X position. Requires SINGLENOZZLE_SWAP_PARK" ?Y "Park Y position. Requires SINGLENOZZLE_SWAP_PARK" ?Z "Z Raise")
  ("M218" "Set Hotend Offset" ?T "Hotend index. Active extruder by default" ?X "Hotend X offset" ?Y "Hotend Y offset" ?Z "Hotend Z offset. Requires DUAL_X_CARRIAGE or SWITCHING_NOZZLE")
  ("M220" "Set Feedrate Percentage" ?B "Back up current factor" ?R "Restore the last-saved factor" ?S "Feedrate percentage")
  ("M221" "Set Flow Percentage" ?S "Feedrate percentage" ?T "Target extruder (requires multi-extruder). Default is the active extruder")
  ...
  ("M226" "Wait for Pin State" ?P "Pin number" ?S "State 0 or 1. Default -1 for inverted")
  ...
  ("M240" "Trigger Camera" ?A "Offset to the X return position. (Requires PHOTO_POSITION)" ?B "Offset to the Y return position. (Requires PHOTO_POSITION)" ?D "Duration to hold down the shutter switch. (Requires PHOTO_SWITCH_POSITION and PHOTO_SWITCH_MS)" ?F "Feedrate for the main photo moves. If omitted, the homing feedrate will be used. (Requires PHOTO_POSITION)" ?I "Shutter switch X position. If omitted, the photo move X position applies. (Requires PHOTO_SWITCH_POSITION)" ?J "Shutter switch Y position. If omitted, the photo move Y position applies. (Requires PHOTO_SWITCH_POSITION)" ?P "Delay after pressing the shutter switch. (Requires PHOTO_SWITCH_POSITION and PHOTO_SWITCH_MS)" ?R "Retract/recover length. (Requires PHOTO_POSITION)" ?S "Retract/recover feedrate. (Requires PHOTO_POSITION)" ?X "Main photo move X position. (Requires PHOTO_POSITION)" ?Y "Main photo move Y position. (Requires PHOTO_POSITION)" ?Z "Main photo move Z raise. (Requires PHOTO_POSITION)")
  ...
  ("M260" "I2C Send" ?A "The bus address to send to" ?B "The byte to add to the buffer" ?R "Reset and rewind the I2C buffer" ?S "Send flag. Flush the buffer to the bus")
  ("M261" "I2C Request" ?A "The bus address to request bytes from" ?B "The number of bytes to request")
  ("M280" "Servo Position" ?P "Servo index to set or get" ?S "Servo position to set. Omit to read the current position")
  ("M281" "Edit Servo Angles" ?L "Deploy angle in degrees" ?P "Servo index to update / report" ?U "Stow angle in degrees")
  ("M290" "Babystep" ?P "Use P0 to leave the Probe Z Offset unaffected. (Requires BABYSTEP_ZPROBE_OFFSET)" ?S "Alias for Z" ?X "A distance on the X axis" ?Y "A distance on the Y axis" ?Z "A distance on the Z axis")
  ...
  ("M301" "Set Hotend PID" ?C "C term (requires PID_EXTRUSION_SCALING)" ?D "Derivative value" ?E "Extruder index to set. Default 0" ?I "Integral value" ?L "Extrusion scaling queue length (requires PID_EXTRUSION_SCALING)" ?P "Proportional value")
  ("M302" "Cold Extrude" ?P "Flag to allow extrusion at any temperature" ?S "Minimum temperature for safe extrusion")
  ("M303" "PID autotune" ?C "Cycles. At least 3 cycles are required. Default 5" ?D "Toggle activation of PID_DEBUG output" ?E "Hotend index (-1 for heated bed). Default 0" ?S "Target temperature" ?U "Use PID result. (Otherwise just print it out.)")
  ...
  ("M405" "Filament Width Sensor On" ?D "Distance from measurement point to hot end. If not given, the previous value will be used. The default startup value is set by MEASUREMENT_DELAY_CM")
  ("M406" "Filament Width Sensor Off")
  ("M407" "Filament Width")
  ("M408" "Report JSON-style response")
  ("M409" "Query object model")
  ("M410" "Quickstop")
  ("M412" "Filament Runout" ?S "Flag to enable or disable Filament Runout Detection. If omitted, the current enabled state will be reported")
  ("M413" "Power-loss Recovery" ?S "Flag to enable or disable Power-loss Recovery. If omitted, the current enabled state will be reported")
  ("M415" "Host Rescue")
  ("M416" "Power loss")
  ("M420" "Bed Leveling State" ?C "Center the mesh on the mean of the lowest and highest points" ?L "Load mesh from EEPROM index (Requires AUTO_BED_LEVELING_UBL and EEPROM_SETTINGS)" ?S "Set enabled or disabled. A valid mesh is required to enable bed leveling. If the mesh is invalid / incomplete leveling will not be enabled" ?T "Format to print the mesh data" ?V "Verbose: Print the stored mesh / matrix data" ?Z "Set Z fade height (Requires ENABLE_LEVELING_FADE_HEIGHT)")
  ...
  ("M569" "Set TMC stepping mode" ?E "Stepping mode for the E0 stepper" ?I "Index for dual steppers. Use I1 for X2, Y2, and/or Z2, and I2 for Z3" ?T "Index (tool) number for the E axis. If not specified, the E0 extruder" ?X "Stepping mode for the X stepper" ?Y "Stepping mode for the Y stepper" ?Z "Stepping mode for the Z stepper")
  ...
  ("M575" "Serial baud rate" ?B "The baud rate to set. Permitted values are:" ?P "Serial Port index. Omit for all serial ports")
  ...
  ("M605" "Dual Nozzle Mode" ?R "Temperature difference to apply to E1.  (Requires DUAL_X_CARRIAGE)" ?S "Select the pin to set for all specified axes" ?X "X distance between dual X carriages.  (Requires DUAL_X_CARRIAGE)")
  ...
  ("M700" "Level plate")
  ("M701" "Load filament" ?L "Extrude distance for insertion (positive value) (manual reload)" ?T "Optional extruder index. Current extruder if omitted" ?Z "Move the Z axis by this distance")
  ("M702" "Unload filament" ?T "Optional extruder number. If omitted, current extruder (or ALL extruders with /docs/configuration/configuration.html#advanced-pause)" ?U "Retract distance for removal (manual reload)" ?Z "Move the Z axis by this distance")
  ("M703" "Configure Filament")
  ("M710" "Controller Fan settings" ?A "Set whether the fan speed is set automatically. When turned off the controller fan speed will remain where it is" ?D "Set the extra duration to keep the fan speed high after motors are turned off" ?I "Set the speed of the controller fan when motors are off" ?R "Reset all settings to defaults. Other parameters can be included to override" ?S "Set the speed of the controller fan when motors are active")
  ...
  ("M890" "Run User Gcode")
  ("M900" "Linear Advance Factor" ?K "The K factor to set for the specified extruder. Unchanged if omitted. Set this value higher for more flexible filament or a longer filament path" ?L "Set the second K factor for the specified extruder. Requires EXTRA_LIN_ADVANCE_K. Note that this factor may be inactive and won't take effect until the next M900 S1" ?S "Select slot and activate the last stored value. Requires EXTRA_LIN_ADVANCE_K" ?T "Extruder to which K, L, and S will apply. Requires EXTRA_LIN_ADVANCE_K")
  ("M905" "Set local date and time")
  ("M906" "TMC Motor Current" ?E "Current for the E0 stepper" ?I "(>=1.1.9) Index for dual steppers. Use I1 for X2, Y2, and/or Z2" ?T "(>=1.1.9) Index (tool) number for the E axis. If not specified, the E0 extruder" ?X "Current for the X stepper" ?Y "Current for the Y stepper" ?Z "Current for the Z stepper")
  ...
  ("M912" "Clear TMC OT Pre-Warn" ?E "Clear all or one E stepper driver Over Temperature Pre-warn flag" ?I "Stepper number to set. If omitted, all specified axes" ?X "Clear X and/or X2 stepper driver Over Temperature Pre-warn flag" ?Y "Clear Y and/or Y2 stepper driver Over Temperature Pre-warn flag" ?Z "Clear Z and/or Z2  and/or Z3 stepper driver Over Temperature Pre-warn flag")
  ("M913" "Set Hybrid Threshold Speed" ?E "Set E0_HYBRID_THRESHOLD and E1_HYBRID_THRESHOLD and E2_HYBRID_THRESHOLD and E3_HYBRID_THRESHOLD to the provided value" ?I "(>=1.1.9) Index for dual steppers. Use I1 for X2, Y2, and/or Z2" ?T "(>=1.1.9) Index (tool) number for the E axis. If not specified, the E0 extruder" ?X "Set X_HYBRID_THRESHOLD and X2_HYBRID_THRESHOLD to the provided value" ?Y "Set Y_HYBRID_THRESHOLD and Y2_HYBRID_THRESHOLD to the provided value" ?Z "Set Z_HYBRID_THRESHOLD and Z2_HYBRID_THRESHOLD to the provided value")
  ("M914" "TMC Bump Sensitivity" ?I "(>=1.1.9) Index for dual steppers. Use I1 for X2, Y2, and/or Z2" ?X "Sensitivity of the X stepper driver" ?Y "Sensitivity of the Y stepper driver" ?Z "Sensitivity of the Z stepper driver")
  ("M915" "TMC Z axis calibration" ?S "Current value to use for the raise move. (Default: CALIBRATION_CURRENT)" ?Z "Extra distance past Z_MAX_POS to move the Z axis. (Default: CALIBRATION_EXTRA_HEIGHT)")
  ...
  ("M928" "Start SD Logging" ?f "File name of log file")
